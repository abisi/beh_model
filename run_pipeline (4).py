...
# FEATURE SET BUILDER
# ─────────────────────────────────────────────────────────────────────────────
def build_feature_sets(features: list, trial_types: list | None = None) -> dict:
    """
    Build a dict of named feature sets:
      - "full"               : all features
      - "loo_{feat}"         : all features except {feat}   (one per feature)
      - "loo_{trial_type}_block" : all features except the block of features
                               associated with {trial_type} (if trial_types given)

    Parameters
    ----------
    features    : base list of feature names, e.g. ["bias","whisker","auditory",...]
    trial_types : optional list of trial-type labels whose associated features
                  should be grouped for a joint leave-one-out block,
                  e.g. ["whisker", "auditory"]

    Returns
    -------
    dict[str, list[str]]  e.g. {"full": [...], "loo_whisker": [...], ...}
    """
    sets = {"full": list(features)}

    # Single-feature leave-one-out (always keep bias)
    for feat in features:
        if feat == "bias":
            continue
        sets[f"loo_{feat}"] = [f for f in features if f != feat]

    # Trial-type block leave-one-out (drop all features whose name contains the type)
    if trial_types:
        for ttype in trial_types:
            block = [f for f in features if ttype in f]
            if not block:
                continue
            sets[f"loo_{ttype}_block"] = [f for f in features if f not in block]
...
def _resolve_root():
    ...
    if "haas" in host:
        return Path("/mnt/lsens-analysis/Axel_Bisi")
    elif host == "SV-07M-005":
        return Path("/Volumes/Petersen-Lab/analysis/Axel_Bisi")
    else:
        return Path(r"\\sv-nas1.rcp.epfl.ch\Petersen-Lab\analysis\Axel_Bisi")
...
CFG = dict(
    ...
    info_path     = _ROOT.parent / "z_LSENS" / "Share" / "Axel_Bisi_Share" / "dataset_info",
    dataset_path  = _ROOT / "beh_model" / "datasets",
    global_path   = _ROOT / "beh_model" / "global_glmhmm" / "models",
    single_path   = _ROOT / "beh_model" / "single_mouse_glmhmm",
...
def fit_and_evaluate(glmhmm, input_train, output_train, input_test, output_test, cfg):
    ...
    def _posteriors_preds_acc(outputs, inputs):
        ...
        acc_result = calculate_predictive_accuracy(outputs, preds)
        if isinstance(acc_result, tuple):
            acc, balanced_acc = acc_result
        else:
            acc, balanced_acc = acc_result, acc_result
        return post, preds, acc, balanced_acc
...
def stage_create_dataset(cfg):
    ...
    dataset_params = {"n_trials_max": cfg["n_trials_max"]}
    dataset = create_behavior_dataset(nwb_list, mouse_info_df, dataset_params)
    logger.info(
        f"Dataset: {len(dataset):,} trials | "
...
def _fit_global_worker(args):
    ...
    if len(data_train) == 0:
        logger.warning(f"  [skip – no data] {tag}")
        return None
...
def stage_fit_global(cfg):
    ...
    scalar_keys = [
        "split_idx", "n_states", "instance_idx", "model_name", "reward_group",
        "ll_train", "ll_test",
        "acc_train", "acc_test",
...
_RG_STR   = {1: "R+", 0: "R-", 2: "R+proba"}
_RG_COLOR = {"R+": "forestgreen", "R-": "crimson", "R+proba": "steelblue"}
...
def _compute_bpt(df: pd.DataFrame) -> pd.DataFrame:
    ...
    first = df.iloc[0]
    preds = first.get("output_train_preds", None)
    if preds is None or not isinstance(preds, (list, np.ndarray)):
        logger.warning("bpt: output_train_preds not available in summary – skipping bpt computation.")
        return df
    ...
    preds_test = first.get("output_test_preds", None)
...
def plot_global_performance(df: pd.DataFrame, figure_path: Path,
    ...
    has_bpt = "bpt_train" in df.columns
        ...
        for row_idx, (y_train, y_test, ylabel) in enumerate(row_metrics):
            for col_idx, (y_col, split_label) in enumerate(
                ...
                sns.pointplot(
                    ...
                    ax=ax, estimator=np.mean, errorbar="se",
                    hue=hue_col, palette=plot_palette, hue_order=hue_order,
                )
    ...
    # 1. Combined: reward_group as hue
    _make_fig(sub,
              hue_col="reward_group",
              hue_order=[rg for rg in ["R+", "R-", "R+proba"] if rg in reward_groups_present],
              plot_palette=_RG_COLOR,
              title_suffix="",
              fname="global_performance_by_rg")
        ...
        for mouse_dir in sorted(single_base.iterdir()):
            ...
            for n_states in sorted(cfg["n_states_list"]):
                ...
                for row_i, split_idx in enumerate(splits_present):
                    ...
                    for col_i in range(n_states):
                        ax = axs[row_i, col_i]
                        ax.bar(range(len(feats)), w[col_i], color=color, alpha=0.75)
